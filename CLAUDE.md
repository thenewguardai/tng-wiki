...
- `agents-md.js` is the single canonical generator. `generateAgentsMd` builds the always-on `AGENTS.md`, deliberately lean (~40 generic lines, issue #52): PREAMBLE, LAYOUT, CITATIONS, a compact `MARKER_LEGEND`, GROUNDING_SUMMARY, GUARDRAILS, plus a per-domain block from the `DOMAIN_SECTIONS` map. Everything heavy is on-demand doctrine emitted by `generateDoctrine` into each wiki's `.tng-wiki/doctrine/` (`DOCTRINE_DIR`): `operations.md` (`OPERATIONS_DOCTRINE` - the canonical rounds / ingest / query / lint procedures, the `_inbox/` capture contract, and the `Types:` log vocabulary `schemaLogTypes` parses), `grounding.md` (`GROUNDING_DOCTRINE` - the full three-layer grounding + reconcile protocol), and `markers.md` (`MARKER_TAXONOMY`). The `## Scope` section sits BELOW the schema fence (hand-authored, seeded by init's scope prompt / `--scope`, appended by `upgrade` when missing - `scopeSection` / `hasScopeSection`). The 2026-04-15 pivot (see `ROADMAP.md` milestone 1) collapsed the former per-agent generators into this one file: `AGENTS.md` is the portable schema every supported agent reads.
