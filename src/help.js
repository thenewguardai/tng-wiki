...
export const COMMANDS = [
  ...
  },
  {
    ...
    summary: 'Register an existing wiki directory - or a monorepo of wikis - in the user registry',
    usage: 'tng-wiki register [path] [--shared | --host [<name>]] [--force] [--name <n>] [--default] [--yes]',
    args: [{ name: 'path', required: false, desc: 'wiki directory, or a monorepo root whose depth-1 children are wikis (default: current directory)' }],
    flags: [
      { name: '--shared', desc: 'stamp the wiki\'s committed manifest sharing: "shared" - maintained from multiple machines' },
      { name: '--host', value: '[<name>]', desc: 'stamp sharing: "host:<name>" (default: this hostname) - other machines will refuse to register it' },
      { name: '--force', desc: 'replace an existing same-slug entry that points elsewhere, or register another host\'s wiki anyway' },
      { name: '--name', value: '<n>', desc: 'override the registry name/slug' },
      { name: '--default', desc: 'set this wiki as the default' },
      { name: '--yes', desc: 'monorepo mode: no prompts - register shared + this host\'s wikis, skip unstamped and other hosts\'' },
    ],
    examples: ['tng-wiki register .', 'tng-wiki register ~/wikis/shared --shared', 'tng-wiki register ~/mattezell-wiki --yes', 'tng-wiki register ~/wikis/research --default'],
  },
  {
