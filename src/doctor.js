...
// Per-registered-wiki schema freshness: the manifest's `schema_version`
// (stamped by init/upgrade) vs the installed CLI. A wiki scaffolded before
// stamping existed reads as version null - `tng-wiki upgrade` both refreshes
// the schema and stamps it. A schema NEWER than the installed CLI also reads
// stale ("fresh" means this CLI's generator wrote it), which correctly nudges
// `npm i -g` first on machines running an older tool.
export function schemaReport(wikis, installed = installedVersion()) {
  return wikis.map((w) => {
    let version = null;
    try {
...
export async function runDoctor(args, deps = {}) {
  ...
  for (const s of schemas) {
    // A wiki living under a temp root is almost certainly a leftover scratch
    // scaffold - schema advice would be noise; the right verb is unregister (#44).
    if (isTempPath(s.path)) {
      console.log(`      ${pc.yellow('⚠')} ${pc.yellow(`${s.slug}: lives under a temp dir - likely an ephemeral scratch wiki; tng-wiki unregister ${s.slug}`)}`);
      continue;
    }
    if (s.fresh) continue;
    const why = s.schema_version
      ? `schema generated by v${s.schema_version} (installed v${version.installed})`
      : 'schema not version-stamped (pre-upgrade scaffold)';
    console.log(`      ${pc.yellow('⚠')} ${pc.yellow(`${s.slug}: ${why} — run tng-wiki upgrade --wiki ${s.slug}`)}`);
  }
