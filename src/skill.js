...
- **\`tng-wiki sources [--uncompiled] [--wiki <slug>]\`** - lists \`raw/\` files, each annotated with how many pages cite it (the \`compiled:\` flag records belief; citations record use). Use \`--uncompiled\` for the ingest queue - lines marked "cited by N" are bookkeeping lag (stamp the flag), the rest are real ingest work. A compiled source "cited by nothing" deserves scrutiny: someone marked it done and no page rests on it.
