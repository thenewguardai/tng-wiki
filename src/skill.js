...
- **\`tng-wiki sources [--uncompiled] [--wiki <slug>]\`** - lists \`raw/\` files. Compiled-state is DERIVED: a source is compiled iff a wiki page cites it, dismissed iff \`tng-wiki dismiss\` recorded a reviewed-nothing-worth-compiling verdict, pending otherwise. There is no flag to stamp and \`raw/\` is never edited; a \`compiled:\` flag in older sources is inert history. \`--uncompiled\` is the real ingest queue; a pending line flagged "legacy compiled:" means someone once marked it done yet no page rests on it - compile it or dismiss it.
