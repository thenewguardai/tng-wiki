...
**Ref pinning.** When an authority has a \`ref\` field set, the user wants this authority read *at that ref* instead of the working tree — typically because the source repo is actively evolving and they want grounding to be deterministic. Know what kind of pin you actually have: a **tag or commit SHA** is a true pin — it freezes the authority to a specific, immutable point in history. A **branch ref** (e.g. \`main\`, \`origin/develop\`) is a *tracking* ref — it moves every time the branch does, so two grounding runs against the same branch ref can see different code. Deterministic grounding against a branch therefore requires recording the SHA the ref resolved to at grounding time (\`git -C <path> rev-parse <ref>\`) alongside your findings — or recommending the user set \`ref\` to that SHA outright. When \`ref\` is set:
