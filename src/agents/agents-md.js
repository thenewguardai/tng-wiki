...
- Warn-level hygiene findings: pages whose file changed (git commit-date, mtime fallback) after their frontmatter \`updated\` (\`frontmatter_updated_stale\` - bump \`updated\`), internal pages referenced in prose instead of \`[[wikilinks]]\` (\`prose_internal_ref\` - apply the suggested wikilink), and \`[[<slug>:page]]\` cross-wiki links whose registered target wiki lacks the page (\`cross_wiki_broken\`; links into wikis not registered on this machine tally into one informational note instead)
